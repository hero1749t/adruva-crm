...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { leadName, assignedToId, assignedToName } = await req.json();

    if (!leadName || !assignedToId) {
    ...
    // TESTING MODE: send only to verified email
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 24px;">
          <div style="background: #1a1b2e; border-radius: 12px; padding: 32px; color: #ffffff;">
            <h1 style="margin: 0 0 8px; font-size: 20px;">🎯 Lead Assigned to You</h1>
            ...
            <div style="background: #252640; border-radius: 8px; padding: 20px; margin-bottom: 24px;">
              <p style="margin: 0 0 8px; font-size: 16px;"><strong>${leadName}</strong></p>
              <p style="margin: 0; color: #9ca3af; font-size: 14px;">Assigned to: <strong style="color: #3b82f6;">${assignedToName || "You"}</strong></p>
            </div>
    ...
    console.error("notify-lead-assigned error:", err);
